...
{
	int g_cost = maze->map[parent].g_cost == -1 ? 1 : maze->map[parent].g_cost + 1;
...
{
	if (EAST(pos) < maze->size && !maze->map[EAST(pos)].blocked && !maze->map[EAST(pos)].close)
		set_open_costs(maze, open_list, EAST(pos), pos);
	if (WEST(pos) >= 0 && !maze->map[WEST(pos)].blocked && !maze->map[WEST(pos)].close)
		set_open_costs(maze, open_list, WEST(pos), pos);
	if (SOUTH(pos) < maze->size && !maze->map[SOUTH(pos)].blocked && !maze->map[SOUTH(pos)].close)
		set_open_costs(maze, open_list, SOUTH(pos), pos);
	if (NORTH(pos) >= 0 && !maze->map[NORTH(pos)].blocked && !maze->map[NORTH(pos)].close)
...
void a_star_loop(maze_t *maze, list_t *open_list)
{
	...
	for (open = open_list; !maze->map[WEST(last_char)].close && !maze->map[NORTH(last_char)].close && open; open = open_list)
	{
		open_list = open_list->next;
		add_sides_to_open_list(maze, &open_list, open->pos);
		maze->map[open->pos].close = true;
	}
	if (!maze->map[WEST(last_char)].close && !maze->map[NORTH(last_char)].close)
	{
